...
export default function BlogRenderer({ content, editorData, className = "", debug = false }: BlogRendererProps) {
    ...
    if (editorData && editorData.trim()) {
      try {
        ...
        if (debug) {
          ...
          console.log('Block types:', parsedData.blocks?.map(b => b.type).join(', ') || 'none')

          // Detailed list analysis
          const listBlocks = parsedData.blocks?.filter(b => b.type === 'list') || []
        ...
        if (debug) {
          ...
          console.log('📋 Lists in converted HTML:', (convertedHtml.match(/<ul|<ol/g) || []).length)
          console.log('📝 List items in converted HTML:', (convertedHtml.match(/<li>/g) || []).length)
        ...
        console.log('Invalid EditorData:', editorData?.substring(0, 200) + '...')
  ...
  React.useEffect(() => {
    if (debug) {
      ...
      console.log('Final lists count:', (htmlContent.match(/<ul|<ol/g) || []).length)
      console.log('Final list items count:', (htmlContent.match(/<li>/g) || []).length)
